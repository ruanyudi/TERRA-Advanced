...
function [solution] = Test_2D(problem_params, uav_data, ugv_data, cfgParams)
    ...
    solution = [];
        ...
        for i=1:cfgParams.iterations
            ...
            [data,txt,raw] = xlsread('F:\\TERRA-main\\a1.xlsx','','','basic');
            data
            raw=raw(2:end,:);
            X = str2double(raw(:,2));
            Y = str2double(raw(:,3));
            X_offset=282891.694;
            Y_offset=3374259.306;
            X=X-X_offset;
            Y=Y-Y_offset;
            X_new = -Y;
            Y_new = X;
            xy = [X_new';Y_new'];
            problem_params.T = xy;
            problem_params.N = size(xy,2);
            problem_params.Home = [min(X_new);min(Y_new)]
            [problem_params] = scene_generator(problem_params,xy);
